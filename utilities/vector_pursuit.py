...
class VectorPursuit:
    ...
    def get_output(self, position: np.ndarray, speed: float, current_tm=None):
        ...
        projected_point = self.waypoints_xy[self.segment_idx] + scale * self.segment
        ...
        while look_ahead_remaining > 0:
            ...
            look_ahead_point = (
                projected_point + look_ahead_remaining * segment_normalised
            )
            ...
            if (
                look_ahead_waypoint == len(self.waypoints) - 2
        ...
        if np.linalg.norm(position - self.waypoints_xy[-1]) < 0.1 or (
            scale >= 1 and self.segment_idx == len(self.waypoints) - 2
        ):
            over = True
